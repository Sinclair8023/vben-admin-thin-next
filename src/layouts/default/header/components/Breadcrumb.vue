<template>
  <div :class="[prefixCls, `${prefixCls}--${theme}`]">
    <el-breadcrumb separator="/">
      <el-breadcrumb-item v-for="route in routes">
        <Icon
          :icon="route.meta.icon"
          v-if="getShowBreadCrumbIcon && route.meta.icon"
        />
        <span v-if="!hasRedirect(route)">
          {{ t(route.meta.title) }}
        </span>
        <router-link
          v-else
          to=""
          @click="handleClick(route, $event)"
        >
          {{ t(route.meta.title) }}
        </router-link>
      </el-breadcrumb-item>
...
export default defineComponent({
  ...
  setup() {
    ...
    function hasRedirect(route: RouteLocationMatched) {
      if (route?.meta?.isLink) {
        return true;
      }

      if (unref(routes).indexOf(route) === unref(routes).length - 1) {
        return false;
      }
      return true;
    }
